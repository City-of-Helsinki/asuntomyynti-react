...
const ApartmentRow = ({ apartment }: { apartment: Apartment }) => {
  ...
  return (
    <div className={css.container}>
      <div className={css.content} style={rowOpen ? { display: 'flex', flex: 5 } : { display: 'flex', flex: 5, margin: 0 }}>
        <div className={css.apartmentCell} style={rowOpen && isMobileSize ? { flex: 2, marginBottom: 24 } : { flex: 2 }} onClick={toggleRow}>
      ...
      {(rowOpen || isDesktopSize) && <div className={css.buttons} style={{ flex: '3 3 0' }}>
        <Button className={css.getToKnowButton} variant={isDesktopSize ? 'supplementary' : 'secondary'}>
          {t('SEARCH:info')}
        </Button>
